...
func newKey(key string) KeyProvider {
...
func (d defaultKeyProvider) GetKey() string {
	return d.key
}
...
func TestCache_GetByLoader(t *testing.T) {
	...
	localKey, _ := cache.GetByLoader(newKey("test"), localLoader)
	assert.Equal(t, "local", localKey)

	cache.Remove("test")

	globalKey, _ := cache.GetByLoader(newKey("test"), globalLoader)
	assert.Equal(t, "global", globalKey)

	cache.Remove("test")

	defaultKey, _ := cache.GetByLoader(newKey("test"), nil)
...
func TestCache_GetByLoaderWithTtl(t *testing.T) {
	...
	key, ttl, _ := cache.GetWithTTL(newKey("test"))
	assert.Equal(t, "global", key)
	assert.Equal(t, ttl, globalTtl)
	cache.Remove("test")

	localKey, ttl2, _ := cache.GetByLoaderWithTtl(newKey("test"), localLoader)
	assert.Equal(t, "local", localKey)
	assert.Equal(t, ttl2, localTtl)
	cache.Remove("test")

	globalKey, ttl3, _ := cache.GetByLoaderWithTtl(newKey("test"), globalLoader)
	assert.Equal(t, "global", globalKey)
	assert.Equal(t, ttl3, globalTtl)
	cache.Remove("test")

	defaultKey, ttl4, _ := cache.GetByLoaderWithTtl(newKey("test"), nil)
	assert.Equal(t, "global", defaultKey)
...
func TestCache_TestMetrics(t *testing.T) {
	...
	cache.Get(newKey("myKey"))
	cache.Get(newKey("myMiss"))
...
func TestCache_TestLoaderFunction(t *testing.T) {
	...
	value, found := cache.Get(newKey("1"))
...
func TestCache_TestLoaderFunctionParallelKeyAccess(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		...
		go func() {
			defer wg.Done()
			value, found := cache.Get(newKey("foo"))
...
func TestCache_ModifyAfterClose(t *testing.T) {
	...
	_, getErr := cache.Get(newKey("broken3"))
...
func TestCache_MultipleCloseCalls(t *testing.T) {
	...
	for now := time.Now(); now.Before(startTime.Add(time.Second * 3)); now = time.Now() {
		if _, err := cache.Get(newKey("test")); err != nil {
...
func TestCache_SkipTtlExtensionOnHit(t *testing.T) {
	...
	for _, err := cache.Get(newKey("expireTest")); err == nil; _, err = cache.Get(newKey("expireTest")) {
	}
}
...
func TestCache_ForRacesAcrossGoroutines(t *testing.T) {
	...
	go func() {
		for i := 0; i < n; i++ {
			...
			go func(i int) {
				time.Sleep(time.Nanosecond * time.Duration(rand.Int63n(1000000)))
				cache.Get(newKey(fmt.Sprintf("test%d", i/10)))
				wgGet.Done()
			}(i)
		}
		wgGet.Done()
...
func TestRemovalWithTtlDoesNotPanic(t *testing.T) {
	...
	value, err := cache.Get(newKey("keyWithTTL"))
	if err == nil {
...
func TestCacheIndividualExpirationBiggerThanGlobal(t *testing.T) {
	...
	data, exists := cache.Get(newKey("key"))
...
func TestCacheGet(t *testing.T) {
	...
	data, exists := cache.Get(newKey("hello"))
...
func TestCacheGetWithTTL(t *testing.T) {
	...
	data, ttl, exists := cache.GetWithTTL(newKey("hello"))
	assert.Equal(t, exists, ErrNotFound, "Expected empty cache to return no data")
	assert.Nil(t, data, "Expected data to be empty")
...
func TestCache_TestGetWithTTLAndLoaderFunction(t *testing.T) {
	...
	_, ttl, err := cache.GetWithTTL(newKey("1"))
	...
	value, ttl, found := cache.GetWithTTL(newKey("1"))
	assert.Equal(t, nil, found)
	assert.Equal(t, "1", value)
